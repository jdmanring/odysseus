#!/usr/bin/env python3
"""Verify the memory / RAG stack (qdrant-client + a local embedding backend) loads.

Run after installing requirements.txt. The local embedding backend powers
semantic memory, RAG, and personal-doc retrieval: llama.cpp (GGUF) by default,
fastembed as the opt-in alternative. The opt-in fastembed path rides on
`onnxruntime`, whose NATIVE runtime has platform prerequisites pip cannot
provide:
...
A silent failure here demotes semantic memory and RAG to keyword search, so run
this check after install — it prints an actionable fix rather than letting the
stack degrade unnoticed at runtime.
...
def main() -> int:
    ...
    # (onnxruntime) is the opt-in alternative and has no build at all on the
    # BSDs. Either backend yields a working stack, so this check is healthy if
