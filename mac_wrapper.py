...
if __name__ == "__main__":
    ...
    # Menu-bar status item. The red button already hides to the Dock and keeps
    # the server alive, so this is purely additive: quick Open / Quit from the
    # menu bar. No "close to tray" toggle — hiding on close is the native macOS
    # convention and the Dock is the system-tray equivalent.
    #
    # Qt 6.11's QSystemTrayIcon does NOT render on macOS 26 (Tahoe): it reports
    # isVisible()==True but no item ever appears. So the primary path is a native
    # Cocoa NSStatusItem via the ObjC runtime (ctypes) — the same mechanism as the
    # fullscreen-exit observer — which attaches to NSStatusBar directly and
    # renders regardless of Qt. The Qt path stays as a fallback for older macOS.
    _tray_keepalive = []

    def _install_native_menubar_tray(on_open, on_quit):
        try:
            import ctypes as C
            objc = C.CDLL("/usr/lib/libobjc.A.dylib")
            objc.objc_getClass.restype = C.c_void_p
            ...
            def nsstr(s):
                return msg(objc.objc_getClass(b"NSString"), b"stringWithUTF8String:",
                           C.c_void_p, (C.c_char_p,), s.encode("utf-8"))

            class CGSize(C.Structure):
                _fields_ = [("width", C.c_double), ("height", C.c_double)]

            # Action target: an NSObject subclass with openOdysseus:/quitOdysseus:
            # that marshal onto the Qt event loop (never touch widgets on the
            # Cocoa thread inline — same rule as the FS-exit observer).
            cls = objc.objc_getClass(b"OdysseusTrayTarget")
            if not cls:
                IMP = C.CFUNCTYPE(None, C.c_void_p, C.c_void_p, C.c_void_p)

                def _open_cb(self_, cmd_, sender_):
                    try:
                        QTimer.singleShot(0, on_open)
                    except Exception:
                        pass

                def _quit_cb(self_, cmd_, sender_):
                    try:
                        QTimer.singleShot(0, on_quit)
                    except Exception:
                        pass

                imp_open, imp_quit = IMP(_open_cb), IMP(_quit_cb)
                _tray_keepalive.extend([imp_open, imp_quit])
                cls = objc.objc_allocateClassPair(objc.objc_getClass(b"NSObject"),
                                                  b"OdysseusTrayTarget", 0)
                objc.class_addMethod(cls, objc.sel_registerName(b"openOdysseus:"),
                                     C.cast(imp_open, C.c_void_p), b"v@:@")
                objc.class_addMethod(cls, objc.sel_registerName(b"quitOdysseus:"),
                                     C.cast(imp_quit, C.c_void_p), b"v@:@")
                objc.objc_registerClassPair(cls)
            target = msg(msg(cls, b"alloc"), b"init")
            _tray_keepalive.append(target)

            # NSStatusItem (variable length) from the system status bar. The bar
            # does not keep a strong ref for us, so retain + keepalive.
            status_bar = msg(objc.objc_getClass(b"NSStatusBar"), b"systemStatusBar")
            item = msg(status_bar, b"statusItemWithLength:", C.c_void_p,
                       (C.c_double,), C.c_double(-1.0))  # NSVariableStatusItemLength
            if not item:
                return False
            msg(item, b"retain")
            _tray_keepalive.append(item)

            button = msg(item, b"button")
            img = None
            for _n in ("icon-512.png", "icon-macos-1024.png"):
                _p = os.path.join(INSTALL_DIR, "static", "icons", _n)
                if os.path.isfile(_p):
                    img = msg(msg(objc.objc_getClass(b"NSImage"), b"alloc"),
                              b"initWithContentsOfFile:", C.c_void_p, (C.c_void_p,), nsstr(_p))
                    break
            if img and button:
                # Size to the menu-bar height; keep the icon's own colour (not a
                # template, which would collapse an opaque icon to a black box).
                msg(img, b"setSize:", None, (CGSize,), CGSize(18.0, 18.0))
                msg(button, b"setImage:", None, (C.c_void_p,), img)
                _tray_keepalive.append(img)
            elif button:
                msg(button, b"setTitle:", None, (C.c_void_p,), nsstr("Odysseus"))

            # Menu (Open / — / Quit) targeting our action object. Assigning a menu
            # makes a click open it; no separate button action needed.
            menu = msg(msg(objc.objc_getClass(b"NSMenu"), b"alloc"), b"init")

            def _add(title, sel_name):
                it = msg(msg(objc.objc_getClass(b"NSMenuItem"), b"alloc"),
                         b"initWithTitle:action:keyEquivalent:", C.c_void_p,
                         (C.c_void_p, C.c_void_p, C.c_void_p),
                         nsstr(title), objc.sel_registerName(sel_name), nsstr(""))
                msg(it, b"setTarget:", None, (C.c_void_p,), target)
                msg(menu, b"addItem:", None, (C.c_void_p,), it)
            _add("Open Odysseus", b"openOdysseus:")
            msg(menu, b"addItem:", None, (C.c_void_p,),
                msg(objc.objc_getClass(b"NSMenuItem"), b"separatorItem"))
            _add("Quit Odysseus", b"quitOdysseus:")
            msg(item, b"setMenu:", None, (C.c_void_p,), menu)
            _tray_keepalive.append(menu)
            print('[LIFECYCLE] native NSStatusItem menu-bar tray installed', flush=True)
            return True
        except Exception as e:
            print(f"[LIFECYCLE] native menu-bar tray install failed: {e!r}", flush=True)
            return False

    def _install_menubar_tray():
        # Deferred to the first event-loop tick (a status item created before the
        # Cocoa run loop starts can fail to attach). Native first; Qt fallback.
        if _install_native_menubar_tray(_show_and_raise, win.request_quit):
            return
        ...
        for _n in ("icon-512.png", "icon-macos-1024.png"):
            ...
            if os.path.isfile(_p):
                _tray_icon = QIcon(_p)
                break
        tray = QSystemTrayIcon(_tray_icon or app.windowIcon(), app)
        tray.setToolTip("Odysseus")
        menu = QMenu()
        menu.addAction("Open Odysseus").triggered.connect(_show_and_raise)
        menu.addSeparator()
        menu.addAction("Quit Odysseus").triggered.connect(win.request_quit)
        ...
        win._tray = tray
        win._tray_menu = menu
        print('[LIFECYCLE] Qt-fallback menu-bar tray installed (visible=%s)'
              % tray.isVisible(), flush=True)
    QTimer.singleShot(0, _install_menubar_tray)
