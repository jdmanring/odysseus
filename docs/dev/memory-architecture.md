...
100x and makes the comparison meaningless, a lesson learned the hard way here.
...
Xeon-W Intel close to these numbers, Core-family Intel around 8 ms (AVX2, no
AVX-512).
...
are: Apple Silicon best (native arm64/NEON/Metal), Xeon-W Intel Macs near our
bench, Core-family Intel Macs (the majority) at the ~8 ms AVX2 tier. OpenBSD
pays a deliberate security tax on top of the VM tax (hardened malloc, measured
at 0.5-2.5 ms below, plus kernel mitigations), which is the price
...
truncation bypassed, since chopping a non-Matryoshka model to 256 would sandbag
...
768-dim embeddings truncated + renormalized at 64 through 768: top-1 is flat
at 0.917 from 64 through 512 (768 recovers the one missed query, noise-level
...
decisiveness peak while cutting vector size and search cost 3x: a justified
operating point. The sweep was later repeated at 140-query resolution on
the full benchmark corpus with the same verdict: R@1 is a noise band
(0.793-0.821) from 128 through 768 dims, so widening past 256 buys nothing
measurable.
...
fastembed's win (~2x) and only matters on one-off reindexes.
...
fat tail (p95 26-35 ms vs 16 or less elsewhere) present in both passes: loopback
...
~1 ms adapter accounts for the 8.7 ms search, so the store adds only ~2.5 ms and the
...
process spun an all-cores OpenMP team (2 procs on the 24-core host: 4.9 ms to
1.3 s per embed, a 260x collapse; app + memory MCP is exactly this 2-process
topology). `n_threads_batch` now defaults to `min(8, cpu)`: single-item
unchanged (5.0 vs 4.9 ms), bulk down 15% (override
...
no OpenMP, and ggml's own spin threadpool livelocks the scheduler when 2x8
threads oversubscribe 12 vCPUs, giving deterministic ~35 s stalls per embed;
at 2x4 it runs a clean 13 ms. OpenBSD therefore caps at `min(4, cpu)`
(costs ~4 ms solo, 7.5 to 11.7 ms; verified live post-deploy at 13.0/13.1 ms
...
- MSVC: `GGML_NATIVE` is a no-op (no `-march=native`); plain build matches the wheel, and
...
(pip's isolated build env pulls cmake/ninja from PyPI, which have no BSD
wheels; use pkg cmake/ninja + venv scikit-build-core), and never pass
...
above 512 *reduces* bulk throughput; 512/512 is the optimum, and there is no knob
...
(onnxruntime, the ONNX model format, and their own tokenizer/stemmer
dependencies) purely to embed. Unifying on llama.cpp means one local-ML engine and one model format
...
fastembed/onnxruntime working, and is now dead weight the fork sheds:
grep-verified, 14 workaround sites across the tree.
...
- **Arch-mismatch onnxruntime wheels** (`setup.py` guards against pip pulling the
  wrong-CPU binary).
- The **fastembed to llama.cpp fallback branching** itself: a two-backend selection
...
- **Model upgrades are one line.** The multilingual path documented below
  ...
  `LLAMACPP_EMBED_FILE`, no code change, which is impossible on fastembed unless they happen
...
- **The community quantizer ecosystem is in reach** (bartowski, mradermacher, and
  the rest): the same library the fork's GGUF-discovery work already taps for LLMs
  now applies to embedders too.
...
- **Embeddings are neither**: a stateless pure function (text to vector, no lock, each
...
- Leading multilingual retrieval (~100 languages) at its size class, competitive
  with models twice its size, which is exactly the axis v1.5 is weak on.
- Same nomic family: same `search_query:` / `search_document:` prefixes and Matryoshka
  768-to-256, so it slots into the existing prefix + 256-dim pipeline with **no lane or
...
- Per-item latency reproduced on an idle host (`tooling/benchmark_embedding_backends.py`):
  ...
  inflates llama.cpp ~100x; the tool now refuses to run at load >2 unless forced.)
