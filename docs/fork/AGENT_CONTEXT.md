...
| `aria2c` | Multi-protocol download utility. Used by the turbo downloader to fetch HuggingFace model weights with 16 parallel connections and resume support. Replaces the unreliable `hf_transfer` Rust accelerator. |
| `tooling/turbo_download.py` | Entry point for the turbo download feature. Chains BinManager → HfUrlResolver → Aria2Wrapper. |
...
| Turbo downloader | `develop` | Complete — upstream PR staged |
