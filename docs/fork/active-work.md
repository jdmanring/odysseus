...
Last updated: 2026-06-20. Memory explosion OOM fix branches built and pushed: `fix/dom-oom-streaming-throttle` (#64, from upstream-mirror) and `fix/dom-oom-phase2-guard` (#65, from fix/dom-oom-virtualization). Issues #64 and #65 filed. Both are upstream-candidate, staged for filing after `fix/dom-oom-virtualization` lands upstream.
...
| [#65 Phase 2 hist===0 stop](https://github.com/jdmanring/odysseus/issues/65) | `fix/dom-oom-phase2-guard` | **Built and pushed.** From fix/dom-oom-virtualization. Replaces hard-stop with _evictLive(count): removes oldest live nodes above viewport, clears intervals/streamRenderer, compensates scrollTop, shows in-place notice. Staged after fix/dom-oom-virtualization lands upstream. |
...
| `fix/dom-oom-phase2-guard` | [#65](https://github.com/jdmanring/odysseus/issues/65) | 3 commits: fix (`d1222f42`), tests (`d62fc2da`), PR draft (`34c385ca`). From fix/dom-oom-virtualization. Phase 2 _evictLive() replaces hist===0 hard-stop. 16 new static-analysis tests added to `tests/test_chat_history_js.py`. PR draft at `docs/fork/upstream/pr-drafts/fix-dom-oom-phase2-guard.md`. File after fix/dom-oom-virtualization lands upstream. |
...
| `fix/dom-oom-virtualization` | [#2](https://github.com/jdmanring/odysseus/issues/2) | 2 commits. Second (`c1f399f5`): Phase 3 BIDI cap changed from DOM-node count to message count — fixes scroll-jump-to-bottom on scroll-up in heavy agent sessions. **Needs in-app verification** with a long agent session. File upstream issue first. |
