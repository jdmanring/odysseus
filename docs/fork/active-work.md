...
Last updated: 2026-06-21. OOM investigation complete (Phase 1). CDP confirmed: Oilpan is the growth source (215K detached nodes vs ~9K live). Five upstream-candidate branches built for issues #68–72. OS integration round complete: Linux CDP+PSI monitor, macOS vm_stat, Windows kernel notification. Senior review pass complete: chatHistory.js `_pruneTop`/`_loadOlder`/rename fixes on `fix/dom-oom-virtualization`. CDP log emits `listeners/node` ratio. All test file AI-boilerplate stripped. Timer leak fixed: issue #73, branch `fix/tool-bubble-timer-leak`, cherry-picked to develop. Verify GC cadence + listener/node ratio in logs after restart. Research doc: docs/fork/memory-explosion-research.md.
...
| `fix/dom-oom-virtualization` | [#2](https://github.com/jdmanring/odysseus/issues/2) | 6 commits. Full three-phase virtualization + Phase 2 _evictLive fix (folded in from deleted `fix/dom-oom-phase2-guard`). 88 static-analysis tests + 572 Playwright tests. PR draft at `docs/fork/upstream/pr-drafts/fix-dom-oom-virtualization.md`. **Needs in-app verification** with a long agent session. File upstream issue first. |
...
| `perf/hljs-deferred-highlight` | [#66](https://github.com/jdmanring/odysseus/issues/66) | Single commit (`a228e3ef`), pushed. Introduces `hljsDefer.js` (IntersectionObserver-based deferred highlighting). Defers `highlightElement()` for off-screen code blocks in `streamingRenderer.js freeze()`, `chatRenderer.js addMessage()`, and `chat.js` agent-round finalization. Immediate highlight kept for user-triggered actions. 5 static-analysis tests. File upstream issue first (issue filed). |
