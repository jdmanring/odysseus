...
Last updated: 2026-06-20. Provider audit complete — 5 new issues filed (#59-#63), 3 new upstream-candidate branches committed and pushed (`fix/provider-logo-ordering`, `fix/nvidia-native-tool-calling`, `fix/api-hosts-provider-gaps`). `feat/longcat-provider` extended with max_tokens override and stream_options fix (commit `fae6ae6d`). `fix/untrusted-tool-result-header` updated with second commit (`484e9702`) that changes header label, adds explicit "use it" instruction, and narrows injection warning to embedded *instructions* only — improved version also applied to develop (`db011acc`). Issue #63 (stream_options compatibility matrix) filed; requires empirical testing before a fix branch can be created. Venice AI and Perplexity max_tokens gaps identified in audit — pending investigation. Memory explosion investigation complete — 3 root causes confirmed; upstream PR #4661 addresses two; research doc at `docs/fork/memory-explosion-research.md`. Two new fix branches needed: `fix/dom-oom-phase2-guard` and `fix/dom-oom-streaming-throttle`.
...
| Issue | Branch | Status |
|-------|--------|--------|
| **Memory explosion** | no branch yet | 3 root causes confirmed. Upstream PR #4661 (open, not merged) fixes thinking-block per-token render + DOM cap + background stream leak. Our work: `fix/dom-oom-phase2-guard` (Phase 2 `hist===0` stop bug), `fix/dom-oom-streaming-throttle` (rAF throttle + StreamRenderer teardown). Research: `docs/fork/memory-explosion-research.md`. Block both branches on upstream PR #4661 ingest. |
