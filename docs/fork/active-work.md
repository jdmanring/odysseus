...
Last updated: 2026-06-21. OOM investigation active. CDP diagnosis overturned V8 old-gen theory — growth is Oilpan detached nodes. All current fixes on develop: --expose-gc + deferred async gc(), hljs deferred highlighting, CDP node-count logging. Two new upstream-candidate branches built and pushed: perf/hljs-deferred-highlight (#66), fix/qtwebengine-oilpan-gc (#67). Pending: verify async GC + hljs defer contain growth after next restart; 7 open research questions in memory-explosion-research.md.
...
| `fix/qtwebengine-oilpan-gc` | [#67](https://github.com/jdmanring/odysseus/issues/67) | Single commit (`20dc1cde`), pushed. Adds deferred `gc({type:'major',execution:'async'})` call 2.5 s after each response's `finally` block in `chat.js`. No-ops when `gc()` absent; requires `--js-flags=--expose-gc` in the launcher (set in `qt_wrapper.py` on `develop`). The `qt_wrapper.py` change is fork-only and excluded from the upstream branch. File upstream issue first (issue filed). |
