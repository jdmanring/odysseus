...
| Issue | Branch | Status |
|-------|--------|--------|
| Issue A — agent context budget locked at 6K | no branch | **Investigation complete. Draft: `docs/fork/upstream/issue-drafts/fix-agent-context-budget-discovery.md`.** Fork issue not yet filed. Root cause: `budget_context_for_model()` returns 0 for any model not in `KNOWN_CONTEXT_WINDOWS`, preventing auto-scaling. 203 confirmed trim events in logs (avg 85.6% context dropped per call). Affects all non-Nemotron NVIDIA NIM models and any unlisted frontier model on api/proxy endpoints. |
| Issue C — 429 treated as hard failure, no backoff | no branch | **Investigation complete. Draft: `docs/fork/upstream/issue-drafts/fix-429-no-backoff-fallback.md`.** Fork issue not yet filed. Root cause: streaming path has zero retries for 429; `stream_llm_with_fallback` advances to next candidate immediately with no delay; `Retry-After` header never read. 2,241 × 429 responses confirmed from NVIDIA NIM (58% failure rate). |
| Issue D — NVIDIA NIM gaps (no curated list, wrong/missing context windows) | no branch | **Investigation complete. Draft: `docs/fork/upstream/issue-drafts/feat-nvidia-nim-support.md`.** Fork issue not yet filed. Root cause: no `"nvidia"` key in `_PROVIDER_CURATED`; 69/91 NIM models unrecognized by `KNOWN_CONTEXT_WINDOWS`; 1 model (`mistral-nemo-minitron-8b-8k`) gets 128K budget despite 8K actual context. NVIDIA's `/v1/models` omits `context_length` fields so live probe cannot fill the gap. |
