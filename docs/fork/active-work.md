...
| `fix/qtwebengine-oilpan-gc` | [#67](https://github.com/jdmanring/odysseus/issues/67) [#69](https://github.com/jdmanring/odysseus/issues/69) | 2 commits: initial deferred GC (`20dc1cde`), stacked-cycle guard + `[GC]` log line (`841670d1`). Adds `gc({type:'major',execution:'async'})` 2.5 s after each response `finally` block; `_gcPending` flag prevents concurrent cycles. Requires `--js-flags=--expose-gc`. File upstream issue first (issues filed). |
| `perf/gc-rendertail-instrumentation` | [#68](https://github.com/jdmanring/odysseus/issues/68) | Single commit (`7ea6ad0f`), pushed. Adds `_rtCalls`/`_rtFast` counters in `streamingRenderer.js`; logs count + fast-path ratio in `finalize()`. Baseline measurement for rAF throttle priority. File upstream issue first (issue filed). |
| `perf/rendertail-raf-throttle` | [#70](https://github.com/jdmanring/odysseus/issues/70) | Single commit (`1a2643bd`), pushed. Guards the "normal streaming" SSE branch with `requestAnimationFrame` so `_renderStream()` fires at most once per frame (~60 fps) instead of once per token. `_streamRenderRaf` cancelled in `finally`. File upstream issue first (issue filed). |
| `perf/streaming-final-render` | [#71](https://github.com/jdmanring/odysseus/issues/71) | Single commit (`fcc4e6df`), pushed. Adds `else if` fast-path in the `[DONE]` handler for plain responses (no thinking, no sources, no findings): calls `finalize()` and unwraps `.stream-content` in-place instead of replacing `_body4.innerHTML`. Falls back to full re-render when refs are absent. File upstream issue first (issue filed). |
| `perf/tool-bubble-inplace` | [#72](https://github.com/jdmanring/odysseus/issues/72) | Single commit (`45b0344e`), pushed. Caches `_toolHeaderEl`, `_toolIconEl`, `_toolWaveEl`, `_toolContentEl` on each bubble node at `tool_start`; at `tool_output`, patches icon text, removes wave + elapsed spans, appends status + chevron in-place — no full `innerHTML` replace on completion. File upstream issue first (issue filed). |
