...
| Issue | Branch | Status |
|-------|--------|--------|
| [#54 agent context budget locked at 6K](https://github.com/jdmanring/odysseus/issues/54) | `fix/agent-context-budget-discovery` | **Implementation in progress.** Root cause: `budget_context_for_model()` returns 0 for any model not in `KNOWN_CONTEXT_WINDOWS`, preventing auto-scaling. Fix: remove `api`/`proxy` early-return from `_query_context_length`; run probe and resolve with priority order. Draft: `docs/fork/upstream/pr-drafts/fix-agent-context-budget-discovery.md`. |
