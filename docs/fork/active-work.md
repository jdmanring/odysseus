...
|--------|----------|-------|
| `fix/dom-oom-streaming-throttle` | [#64](https://github.com/jdmanring/odysseus/issues/64) | Single commit (`d35f3819`), from upstream-mirror. Thinking-block textContent throttle (O(n²) fix), rAF throttle for normal streaming, StreamRenderer teardown, background stream cleanup, idle GC yield. File upstream issue first. |
| `fix/dom-oom-phase2-guard` | [#65](https://github.com/jdmanring/odysseus/issues/65) | Single commit (`d1222f42`), from fix/dom-oom-virtualization. Phase 2 _evictLive() replaces hist===0 hard-stop. File after fix/dom-oom-virtualization lands upstream. |
