...
### Cookbook / Download
- **Structured download card:** Replaced the raw `<pre>` terminal output for download tasks with a purpose-built progress card showing phase (initializing → resolving → downloading → paused → done/error), progress bar, per-file index, size, speed, ETA, and connection count badge. All phase sections live in the DOM; CSS `[data-dl-phase]` selects which is visible — no re-render flicker.
- **Parallel file downloads:** `aria2c_download.py` now uses aria2c's `--input-file` mode to download up to 4 files simultaneously (4 connections each, 16 total — same bandwidth budget as the old sequential approach). Multi-shard models (e.g. 4× 4.6 GB shards) complete significantly faster end-to-end since large files can download in parallel.
...
- **Live in-place DOM updates:** `_updateDownloadCard()` is called on every `_reconnectTask` poll tick; only the changed attributes are touched — no `innerHTML` replacement mid-stream. Guard added so the updater is a no-op for paused tasks.
