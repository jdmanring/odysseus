...
re-trim until the next message (n=5000: 39k nodes / 122.9 MB USS, about equal to unpatched) and loses
...
a live model, so it goes on the 3.5 manual-smoke checklist.
...
- Evicted-message reload speed: **MEASURED.** Handler ~2 ms/page; deep scroll-back cost is
  serialized pages x RTT (network arm). Flash-on-reload: MANUAL, needs eyes on a real
  session.
