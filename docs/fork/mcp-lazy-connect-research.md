...
- `get_tool_descriptions_for_prompt` / `get_all_openai_schemas` **skip built-in Python
  ...
  agent prompt"*, quoted verbatim); only the NPX browser server is advertised dynamically.
...
1. **`builtin_mcp._EAGER_SERVERS = {"memory"}`.** `register_builtin_servers` connects eager
   servers as before; for the rest it calls `mcp_manager.mark_deferred(server_id, name)`, which
