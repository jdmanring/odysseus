...
runtime bookkeeping: a windowed slice *cannot* delete a live agent thread or tool timeline (only
the rendered range is ever in the DOM, so there is nothing to mis-trim), and it *cannot*
