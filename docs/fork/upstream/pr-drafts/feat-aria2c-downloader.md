...
- Spawns aria2c with 4 parallel files × 3 connections per file (12 total
  ...
  others. 12 was chosen to match the aggressive parallel throughput intent of
  `hf_transfer` (its stated purpose is high-speed multi-threaded transfer via
  10 MB parallel chunk reads) while staying within aria2c's documented
  16-connection ceiling. Users on constrained networks can reduce
