...
- Spawns aria2c with 4 parallel files × 3 connections per file (12 total
  ...
  maximum is 16. 12 was chosen to match the aggressive parallel throughput
  that `hf_transfer` provided by default (its stated purpose is high-speed
  multi-threaded transfer). Users on constrained networks can reduce
