...
**PII redaction:** Key-name-based sensitive data redaction (Sentry-style denylist) that matches field names like `email`, `password`, `token`, `content`. Never scans string values, so legitimate string content isn't accidentally censored.
