# Issue Draft: feat/skill-quality-signals → pewdiepie-archdaemon/odysseus
...
- `_compute_idf(skills)` builds an inverse document frequency table over the query-
  candidate corpus. O(N × L) where N = skill count, L = average token count per skill.
...
|--------|--------|--------------|
| `confidence` | 0–40 | frontmatter `confidence` × 40 |
