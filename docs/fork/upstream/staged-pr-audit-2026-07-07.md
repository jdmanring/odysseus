...
| Branch | Blocker |
|---|---|
| **fix/dom-oom-virtualization** | Provenance framing: plan Part 1.2 (settled: independent architecture authored nine days before #4661; teardown described by what it does; #4661 = parallel work; no attribution owed or claimed). Remaining blockers: ~2430 lines / trial-and-error history -> rebuild per the plan's Part 5 gates. (Upside: the *only* branch with a real Playwright behavior test.) |
...
- **fix/skill-extraction-threshold vs fix/skill-lifecycle-correctness**: **mutually exclusive**: flip `auto_approve_skills` default in *opposite* directions at the same three sites. At most one can land; pick one semantics.
- **feat/skill-quality-signals** has an **IDF-cache staleness bug**: `_idf_cache` computed over the first corpus, applied to later different corpora (per-owner/status subsets) -> silently wrong BM25 rankings; not covered by tests.
...
**Fixed:**
- **Skills mutual-exclusion (#84 vs #86)**, resolved: **#84's `auto_approve_skills`
...
- **perf/chathistory-gc-improvements: PROVEN REDUNDANT (retire).** Not "diagnosed" but verified:
