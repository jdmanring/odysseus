...
| `fix/dom-oom-virtualization` | [#2](https://github.com/jdmanring/odysseus/issues/2) | Bug | **Independently-developed, more-complete alternative to open upstream PR #4661.** Verified parallel work: our first commit is ~19h before #4661 was opened, and our code does not use theirs. Ours adds bidirectional scroll-up and releases StreamRenderer/IntersectionObserver/hljs-defer refs that #4661 leaves attached; #4661 is smaller. Offer on merits, respectfully. **Before filing: remove the `docs/fork/` PR-draft file committed on the branch (contamination).** See pr-drafts/ relationship note. |
