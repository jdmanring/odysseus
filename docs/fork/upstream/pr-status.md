...
| `fix/dom-oom-virtualization` | [#2](https://github.com/jdmanring/odysseus/issues/2) | Bug | **More-complete alternative to open upstream PR #4661, with an independent architecture.** Provenance (RESOLVED 2026-07-18, plan Part 1.2, primary-source): architecture AUTHORED 2026-06-11 (author dates) — **nine days before #4661 opened** (the old "19h" figure was a rebase commit-date). The `_evictLive` teardown makes **no influence claim in either direction** (authored ~5h after #4661 opened; only overlap is clearing the app's own timer handles, forced on any removal path; #4661's mechanism never used). Do NOT claim either full teardown-independence OR adaptation — both overclaim. Ours adds bidirectional scroll-up and releases StreamRenderer/IntersectionObserver/hljs-defer refs beyond #4661; #4661 is smaller. Offer on merits, acknowledging #4661 as parallel work. Branch contamination (PR-draft .md) removed. **Plan: `docs/fork/plans/dom-oom-virtualization-upstream-plan.md` — its Part 4.5 is mandatory: the server-paging fold must carry the #129 chIdx-retag fix (`ac18291a`) and its regression test, or the filed PR ships a measured unbounded-DOM defect. Fork #129 stays open until this PR is filed.** |
