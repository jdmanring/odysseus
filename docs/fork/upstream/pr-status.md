...
| `fix/slash-memory-category-escape` | **NEW branch (#182)**, staged 2026-08-03, **rewritten 2026-08-04 after four independent reviews falsified its central claim.** As first written it reported an XSS whose reproduction does not fire: it claimed `category` is never validated server-side, when `MemoryAddRequest` (`src/request_models.py:42-47`) coerces anything outside a seven-value allowlist to `fact`. Two further claims were also false -- "sole output path" (`_eggRender` at `slashCommands.js:5314` is a second `innerHTML` sink with 10 callers) and "single-user, local-first" (real multi-user auth; memory is *owner-scoped*, which is the correct reason severity stays low). Now framed as what it is: defence in depth over four fields left raw beside escaped neighbours, one of them in an `href` attribute. Two of the four were found by the review, in the same file the first commit had just edited. Guards rewritten too -- the originals asserted four source literals and passed a variant carrying three live XSS holes. **Revised again 2026-08-04 (#187, #188), tip `6234e1f8`:** the replacement guard was itself defeated by seven live variants, so it now EXECUTES each template under node rather than scanning source; that found 18 further raw fields beyond the four, all escaped. Both commit messages were rewritten so the retracted claims are not published. Independent of #184 -- no common files, applies in either order. |
| `fix/compact-json-for-model-payloads` | #190 | **Staged 2026-08-04, tip `8be59552`.** Three formatters `json.dumps(indent=2)` into the model's context then truncate at a char cap, so whitespace displaces real data: a 40-event calendar payload is 9905 chars pretty vs 6937 compact and is cut off mid-object today. 30-41% of tokens measured with tiktoken on real payload shapes. Lossless (`json.loads(compact) == original`). `integrations.py`'s binary-search truncation now fits 69 items in 11845 chars where it managed 53. Six mutations verified to fail, including both halves of a measure/emit mismatch that parsing alone does not catch. Cherry-picked to develop `896f6c2b` (patch-id `155bbe75`). |
...
**#182 and #184 were both rewritten on 2026-08-04** after a four-lens adversarial
review falsified load-bearing claims in each (#186, #187, #188). Both are now fixed and
their drafts rewritten to match what shipped. Branch tips moved: `fix/memory-category-validation`
is `bd3036ab`, `fix/slash-memory-category-escape` is `6234e1f8` (its two commit messages
were rewritten, so the earlier SHAs no longer exist on the branch; rollback ref at
`refs/premsg/slash-memory-category-escape`).

They are **independent**: the branches touch no common file and apply in either order.
The earlier "file #184 first" instruction was a preference stated as a constraint.
