...
| `fix/dom-oom-virtualization` | [#2](https://github.com/jdmanring/odysseus/issues/2) | Bug | **HOLD, do not file as-is.** Competes with open upstream PR #4661 (pagination vs full virtualization for the same DOM-bounding problem). Let #4661 land, then decide whether virtualization adds value on top; if so propose as an enhancement, not a replacement. See pr-drafts/ relationship note. |
