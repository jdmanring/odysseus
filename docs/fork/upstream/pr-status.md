...
| `feat/qt-native-linux-app` | [#14](https://github.com/jdmanring/odysseus/issues/14) | Feature | **Code complete; NOT yet file-ready — two hard pre-file gates below.** Now carries the full Qt-wrapper memory stack (#106 forcible-purge + RSS-ceiling, #112 host telemetry, #116 low-resource profile, #120 graduated PSI / `qt_psi.py`), folded 2026-06-26 because `qt_wrapper.py` is a *new* file (ships memory-managed, not patched after). 13 commits cherry-picked onto the branch (clean; backup `backup/feat-qt-native-linux-app-prefold`); invariant verified — `git diff develop feat/qt-native-linux-app -- qt_wrapper.py qt_psi.py` is theme-bg only. PR draft rewritten to the shipped design. **Gate 1 (history):** the fold replayed the *evolution*, so the 15-commit history shows simulatePressure and `--enable-low-end-device-mode` added-then-removed and the PSI monitor rewritten — the naive-then-patched narrative a commit-reading reviewer would see. **Squash to coherent commits at file time** (against the then-current `upstream-mirror` base, per the pre-file rebase requirement); prove it changed only history with `git diff <squashed> feat/qt-native-linux-app` == empty. **Gate 2 (verification):** the `CRITICAL → _purge_renderer` path is unconfirmed in-app — the stress-ng smoke only reached MODERATE and the headless `qt_psi` harness can't touch `_purge_renderer` (window method). The human filer must confirm the `[MEM] forcible purge (psi-critical)` line actually fires (How-to-Test step 8) before checking the "ran end-to-end" box. See pr-drafts/feat-qt-native-linux-app.md |
