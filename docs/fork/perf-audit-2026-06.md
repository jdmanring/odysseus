...
axis. A 4000x3000 photo is ~48 MB decoded regardless of file size.
...
- **Per-step magnitude is the real cost:** `_snapshotState()` captures **every layer's
  ...
  them). One step = Σ(all layers' full RGBA bitmaps), x20. Worst case bounded but large:
  4000x3000 x 3 layers x 20 is ~2.9 GB; 2000x1500 x 2 x 20 is ~480 MB. The waste: most
...
| PNG-compress snapshots (lossless) | ~5-10x | undo *latency* (visible, recoverable) | low |
...
| QtWebEngineProc `--type=zygote` x2 | 68 + 68 | 18 + 24 | 0.9 + 13.5 | **Nearly free: shared COW fork templates. Leave alone.** |
...
internally (about `IsLowEndDevice()` + system RAM) with no command-line cap. The only flag that
...
That means they are contribution-worthy, not fork-local quirks.
