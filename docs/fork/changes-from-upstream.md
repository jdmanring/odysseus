...
| File | Purpose |
|------|---------|
| `tooling/aria2c_download.py` | Download entry point. Resolves HF signed URLs -> writes `--input-file` -> spawns single aria2c subprocess. 4 files x 16 connections = 64 total. Resume via `.aria2` sidecar files. No daemon, no RPC. |
