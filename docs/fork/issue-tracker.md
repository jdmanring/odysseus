...
| [#152](https://github.com/jdmanring/odysseus/issues/152) | Per-frame decoration cost: full-screen backdrop-filter flicker (QtWebEngine), standing will-change layers | upstream-candidate | **folded into `fix/gpu-compositor-flicker`** (same theme, June branch; `8d2ea99f`..`856dfbe4`; develop `675fe400`, `be191d9d`, `8613e26a`, `10759b67`). Note: the clear-all flicker was a develop *regression* — upstream's cookbook/gallery commit (`92daf4e5`) re-added blur the June branch had removed; the branch's guard test was branch-only, so develop had no tripwire. Develop's new `test_css_no_fullscreen_backdrop_blur.py` closes that. |
