...
| [#121](https://github.com/jdmanring/odysseus/issues/121) | Installed models don't grey out: duplicated downloaded-detection matcher | upstream-candidate | **Implemented** on `fix/model-downloaded-detection` (from `upstream-mirror`, `c17973f2`); cherry-picked to develop. One canonical `static/js/model/downloaded.js` predicate; all sites consolidated; node test locks the better-quant case + guard test blocks reintroduction. Plan: `docs/fork/plans/model-downloaded-detection-consolidation.md` |
