...
| [#153](https://github.com/jdmanring/odysseus/issues/153) | vLLM serve fails on desktop/toolkit-less machines: free-VRAM refusal undiagnosed; flashinfer JIT requires nvcc | upstream-candidate | `fix/vllm-desktop-serve-resilience` (develop `5c76ad32`, `cd3f182f`; branch carries a serve-specific test file, the develop guard tests live in the downloader-themed file, which stays with `feat/aria2c-downloader`) |
| [#159](https://github.com/jdmanring/odysseus/issues/159) | Desktop tray: server-runner menu + native About dialog (Linux/Windows/macOS) | upstream-candidate | staged on the per-OS wrapper branches (`feat/qt-native-{linux,windows,macos,freebsd}-app`). Also folded under #159: the About dialog upgrade, Windows dark title bar, the three qt_wrapper BSD-hardening fixes (force software render, guard QtDBus import, disable renderer purge when `/proc` RSS is unreadable, the FreeBSD **white-screen** crash-loop), and the FreeBSD dependency codification. develop `89db8864` |
| [#160](https://github.com/jdmanring/odysseus/issues/160) | Installers: verify the memory/RAG stack at install time + auto-install native prerequisites (Windows VC++ redist) | upstream-candidate | `feat/verify-memory-stack` (`0686579b`; `tooling/verify_memory_stack.py` + `setup.sh`/`setup.ps1` hooks; develop `89db8864`) |
| [#161](https://github.com/jdmanring/odysseus/issues/161) | Memory overhaul: Qdrant vector store + fully-optimized nomic (256-dim MRL, prefixes, sized chunks) | upstream-candidate | `feat/memory-qdrant-nomic` (single clean commit off `upstream-mirror`; full suite green 4687-0; **stacks on `feat/logging`** for `structlog`; modifies #160's `verify_memory_stack.py` for qdrant-client; develop carries the loose originals, superseded by the clean branch commit). Pending: Qdrant binary lifecycle + installer wiring. |
