...
# ── Icon (best effort) — center-crop assets/landingpage.jpg to a square .icns ──
if [ -f "$REPO_DIR/static/icons/icon-512.png" ] && command -v sips >/dev/null 2>&1; then
  ...
  sips -c 512 512 "$REPO_DIR/static/icons/icon-512.png" --out "$TMPIMG/sq.png" >/dev/null 2>&1 || cp "$REPO_DIR/static/icons/icon-512.png" "$TMPIMG/sq.png"
  ...
  echo "  icon:        (skipped — no static/icons/icon-512.png)"
fi
