...
def test_guard_gates_on_visibility_not_just_connected():
    # isConnected is true for display:none, so the guard must also require the
    # element to render a box. Use getClientRects (correct for position:fixed),
    # NOT offsetParent (null for a visible element in a fixed container).
    body = _guard_body()
