"""
Verify the extraction gate thresholds introduced by
fix(skills): raise extraction threshold, align confidence floor.

Gate now requires rounds >= 2 AND tools >= 3 (was rounds >= 2 OR tools >= 2).
MIN_CONFIDENCE raised from 0.6 to 0.85 (aligned with the injection floor, so a
skill that could never be injected is not extracted in the first place).

Note: auto_approve_skills / draft-vs-publish semantics are owned by
fix/skill-lifecycle-correctness (#86), not this branch.
"""
