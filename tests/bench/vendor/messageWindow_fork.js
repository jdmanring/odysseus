...
//   blob eac4f381866267a0886a14226f3caca690eb6871
//
// WHY THIS IS VENDORED, AND WHAT IT IS NOT
// ----------------------------------------
// The other two non-trivial arms (#4998's chatVirtualizer, the hybrid) are
// vendored snapshots; this arm was not -- it reached into the live working tree.
// That made the benchmark unrunnable on any branch lacking chatHistory.js, which
// is every branch except `develop`. Vendoring makes the bench deterministic and
// reproducible from a clean checkout.
//
// IMPORTANT, AND EASY TO MISREAD: this file is *not* the eviction implementation
// staged for upstream. The staged PR (`fix/chat-history-dom-eviction`) implements
// eviction as ~112 lines of functions in static/js/sessions.js
// (_evictHistoryOverflow / _tagHistoryOffset / _teardownHistoryNode) hooked into
// upstream's existing _installHistoryPager. THIS file is the fork's 1090-line
// MessageWindow class. Both remove DOM nodes and refetch from the server, and that
// shared mechanism is what the `evict` arm measures -- but they are distinct code,
// and no number produced from this file may be attributed to the code in that PR.
// See docs/dev/chat-history-benchmark.md, "What each arm actually is".
//
// tests/test_bench_vendor_snapshot_drift.py asserts this stays identical to the
// live chatHistory.js wherever that file exists, so the snapshot cannot rot.
