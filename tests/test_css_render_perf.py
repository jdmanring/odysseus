...
def test_sidebar_no_contain():
    # .sidebar is opaque with overflow:hidden — no paint isolation is needed, and
    # even `contain: layout` creates a stacking context that alters how its
    # box-shadow separator renders (visually expanded); `contain: content` (paint)
    # additionally breaks the frosted-theme backdrop-filter. So .sidebar carries
    # NO containment (matches develop / fix/css-contain-paint-transparent).
...
def test_chat_history_has_contain_layout_style():
    # Scope recalculation on addMessage() appends, but NOT paint — paint containment
    # renders the transparent chat area opaque under the low-end tile budget. Use
    # `contain: layout style` (matches fix/css-contain-paint-transparent).
...
def test_modal_content_has_contain_layout_style():
    ...
    # Line-anchored (newline + indent + bare selector): skips scoped overrides
    # like `#cookbook-modal .modal-content` that upstream added before this rule.
    block = _block("\n    .modal-content {")
    rule_end = block.index("}")
    assert "contain: layout style" in block[:rule_end]
