...
at some point in the past. When upstream-mirror advances, the staging branch needs to
be rebased so our commit(s) apply cleanly on top of the current upstream code.
...
```
Then stop and ask James before proceeding.
...
If any check fails, fix it before updating pr-status.md.
...
When in doubt about any git operation: stop, describe what you're about to do, and
ask James to confirm before executing.
