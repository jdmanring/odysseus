...
def _classify_agent_request(messages: List[Dict], last_user: str) -> Dict[str, object]:
    ...
    if has(
        ...
        domains.add("files")
    # Fork: also match filesystem paths and source-file extensions the keyword
    # lists above miss (e.g. "/home/…", "~/…", "foo.py", "codebase", "browse").
    if has(r"\b(files?|folders?|director(?:y|ies)|repo(?:s|sitory|sitories)?|filesystem|codebase|source code|browse)\b|/home/|/usr/|/etc/|~/|\.(py|js|ts|json|yaml|yml|toml|sh|cfg|ini|md|txt)\b"):
        domains.add("files")
