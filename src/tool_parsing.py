...
def parse_tool_blocks(text: str, skip_fenced: bool = False) -> List[ToolBlock]:
    ...
    # Pattern 4e: <tool_code> blocks (Google Gemma pycall style) [fork #35]
    if not blocks:
    ...
    # Pattern 4f: <longcat_tool_call> blocks (Meituan LongCat — JSON only) [fork #38]
    if not blocks:
...
def strip_tool_blocks(text: str, skip_fenced: bool = False) -> str:
    ...
    # Fork: strip pycall-form <tool_code>…</tool_code> (upstream's _TOOL_CODE_OPEN_RE
    # requires a "{" so it misses the pycall form #35) and LongCat blocks (#38).
    cleaned = _TOOL_CODE_ANY_RE.sub('', cleaned)
