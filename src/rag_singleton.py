...
def get_rag_manager():
    """Lazy Qdrant-backed VectorRAG initializer.

    Returns the VectorRAG instance on first successful init, None if the vector
    store isn't reachable / available. Failed init attempts are throttled to once
    per _RETRY_INTERVAL seconds so a missing store doesn't busy-retry on
    ...
    try:
        ...
        if not rag_instance.healthy:
            ...
            logger.info("Initialized VectorRAG with Qdrant")
